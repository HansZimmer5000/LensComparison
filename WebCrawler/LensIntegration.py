...
class LensIntegration:
    ...
    def __init__(self, new_lenses):
        self.default_lens = new_lenses.pop(0)
        self.keys = self.default_lens.keys()
        self.new_lenses = new_lenses

    def integrate(self):
        #TODO: Make better!

        if(len(self.new_lenses) != 0):
            for key in self.keys:
                self.__set_new_value_if_existing_is_empty(key)

        result_lens = self.default_lens
        return result_lens

    def __set_new_value_if_existing_is_empty(self, key):
        old_value = self.default_lens[key]
        if(old_value == ""):
            self.__set_first_not_empty_new_value(key)

    def __set_first_not_empty_new_value(self, key):
        for new_lens in self.new_lenses:
